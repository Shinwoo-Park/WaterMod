...
Here’s the simple, two-step process at each decoding step:
    ...
    - Rank: The model first calculates the probability for every token. We sort these tokens by their probability, from most likely (Rank 1) to least likely (Rank V).
    - Partition: We apply a simple modular arithmetic rule: rank mod k. This rule splits all tokens into k different color classes based on their rank.
